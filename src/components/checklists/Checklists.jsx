  ...
  const modosDisponibles = [
    {
      ...
      label: '🔧 COMPLETO',
      icono: '📋',
      color: 'linear-gradient(135deg, #0f2b5e, #1a4c9e)',
    },
    {
      id: 'fluidosLuces',
      label: '💧💡 FLUIDOS + LUCES',
      icono: '🔆',
      color: 'linear-gradient(135deg, #065f46, #b45309)',
    },
    {
      id: 'herramientasErasCajasEquipos',
      label: '🔧🎽📦🧯 HERRAM. + ERAs + CAJAS + EQUIPOS',
      icono: '⚙️',
      color: 'linear-gradient(135deg, #1e3a8a, #6b21a5)',
    },
    {
      id: 'fluidos',
      label: '💧 FLUIDOS',
    ...
    },
    {
      ...
      label: '🎽 ERAs + MÁSCARA',
      icono: '🎭',
      color: 'linear-gradient(135deg, #6b21a5, #9333ea)',
    },
    {
      id: 'equipos',
      label: '🧯 EQUIPOS',
  ...
  const exportarPDF = (cl) => {
    ...
    if (cl.cajas) {
      Object.entries(cl.cajas).forEach(([, caja]) => {
        ...
        addSection(
          `🧰 Caja: ${caja.nombre}`,
          ['Item', 'Cantidad', 'Estado', 'Observaciones'],
          cajaItemsData
        );
      });
    }
    ...
    return (
      ...
      >
        ...
        >
          ...
          <p>
            <strong>🚛 Vehículo:</strong> {cl.vehiculoNombre}
          </p>
          <p>
            <strong>📅 Fecha/Hora:</strong> {cl.fecha} {cl.hora}
          </p>
          <p>
            <strong>👤 Usuario:</strong> {cl.usuario}
          </p>
          <p>
            <strong>📋 Tipo:</strong> {cl.tipo}
          </p>
          <p>
            <strong>
          ...
          {cl.observaciones && (
            <p>
              <strong>💬 Observaciones generales:</strong> {cl.observaciones}
            </p>
          )}

          {Object.keys(cl.fluidos || {}).length > 0 && (
            <>
              <h4 style={{ ...styles.cardTitle, color: '#bbd4ff' }}>
                🛢️ Fluidos
              </h4>
              <table
                style={{
                ...
                <tbody>
                  {Object.entries(cl.fluidos).map(([key, val]) => (
                    <tr key={key}>
                      ...
                      <td style={{ color: val.ok ? '#4ade80' : '#f87171' }}>
                        {val.ok ? '✓ OK' : '✗ NO OK'}
                      </td>
                ...
                <tbody>
                  {Object.entries(cl.items).map(([, val]) => (
                    <tr key={val.nombre}>
                      <td>{val.nombre}</td>
                      <td>
                        {val.cantidadReal}/{val.cantidadEsperada} {val.unidad}
                      </td>
                      <td style={{ color: val.ok ? '#4ade80' : '#f87171' }}>
                        {val.ok ? '✓ OK' : '✗ NO OK'}
                      </td>
  ...
  if (vista === 'lista') {
    return (
      <div>
        ...
        >
          <h2 style={{ ...styles.pageTitle, color: '#e2e8f0' }}>
            📋 Checklists
          </h2>
          <button
            style={{
              ...
              >
                <div>
                  ...
                  <small style={{ color: '#94a3b8' }}>
                    Tipo: {cl.tipo} | Resultado:{' '}
                    {cl.resultado === 'ok' ? '✅ OK' : '⚠️ Con novedades'}
                  </small>
                </div>
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    if (window.confirm('¿Eliminar?')) onEliminar(cl.id);
                  }}
                  style={{
                    ...styles.btnPrimary,
                    background: '#ef4444',
      ...
      {!vehiculoSel ? (
        ...
        <div>
          ...
          >
            ...
            >
              <h3 style={{ color: '#aaffff' }}>
                📋 Checklist para {vehiculoSel.nombre}
              </h3>
              <button
          ...
          >
            <h3 style={{ color: '#aaffff', marginBottom: 16 }}>
              ...
              {modosDisponibles.map((modo) => (
                <button
                  ...
                  style={{
                    padding: '10px 20px',
                    background:
                      tipo === modo.id ? modo.color : 'rgba(30,40,70,0.8)',
                    color: 'white',
                    border:
                      tipo === modo.id
          ...
          {(tipo === 'fluidos' ||
            ...
            >
              ...
              {fluidosConfig.map((fc) => {
                const val = estadoFluidos[fc.key] || {
                  ok: null,
                  observaciones: '',
                };
                return (
          ...
          {(tipo === 'herramientas' ||
            tipo === 'completo' ||
            tipo === 'herramientasErasCajasEquipos') &&
                ...
                {Object.entries(estadoItems).map(([itemId, item]) => (
                  ...
                  >
                    <div>
                      ...
                      - Esperado: {item.cantidadEsperada} {item.unidad} -
                    ...
                    >
                      {renderOkNokButtons(item, setEstadoItems, itemId, 'ok', {
                        cantidadReal:
                          item.ok === true
            ...
            Object.keys(estadoERAs).length > 0 && (
              ...
              >
                ...
                {Object.entries(estadoERAs).map(([eraId, era]) => (
                  ...
                  >
                    <div>
                      <strong style={{ color: '#e2e8f0' }}>{era.nombre}</strong>{' '}
                      - Serial: {era.serial}
                    </div>
                    <div
                      style={{
                    ...
                    >
                      <span style={{ fontWeight: 'bold', color: '#bbd4ff' }}>
                        🎭 Máscara:
                      </span>
                      {renderOkNokButtons(
                        era,
                ...
                {Object.keys(estadoERAs).length === 0 && (
                  <p style={{ color: '#94a3b8' }}>
                    No hay ERAs asignadas a este móvil
                  </p>
                )}
              </div>
            )}
          ...
          {(tipo === 'equipos' ||
            tipo === 'completo' ||
            tipo === 'herramientasErasCajasEquipos') &&
            Object.keys(estadoEquipos).length > 0 && (
              <div
                style={{
                  ...styles.card,
                  background: 'rgba(15,25,45,0.7)',
              ...
              >
                ...
                {Object.entries(estadoEquipos).map(([eqId, eq]) => (
                  ...
                  >
                    <div>
                      <strong style={{ color: '#e2e8f0' }}>{eq.nombre}</strong>{' '}
                      - Código: {eq.codigoInterno} - Vence: {eq.vencimiento}
                    </div>
                    <div
                      style={{
                        display: 'flex',
                ...
                {Object.keys(estadoEquipos).length === 0 && (
                  <p style={{ color: '#94a3b8' }}>No hay equipos asignados</p>
                )}
              </div>
            )}
          ...
          {(tipo === 'cajas' ||
            tipo === 'completo' ||
            tipo === 'herramientasErasCajasEquipos') &&
            Object.keys(estadoCajas).length > 0 && (
              <div
                style={{
                  ...styles.card,
                  background: 'rgba(15,25,45,0.7)',
              ...
              >
                ...
                {Object.entries(estadoCajas).map(([cajaId, caja]) => {
                  ...
                  const algunaNok = caja.items.some(
                    (item) => item.ok === false
                  );
                  const estadoCaja = todosOk
                    ? '✅ OK'
                    : algunaNok
                    ? '⚠️ CON NOVEDADES'
                    : '⬜ SIN VERIFICAR';
                  const estadoColor = todosOk
                    ? '#10b981'
                    : algunaNok
                    ? '#ef4444'
                    : '#9ca3af';
                  return (
                    ...
                    >
                      ...
                      >
                        <div>
                          <strong style={{ fontSize: 15, color: '#aaffff' }}>
                            🧰 {caja.nombre}
                          </strong>
                          {caja.codigoInterno && (
                            <span
                      ...
                      {caja.expanded && (
                        <div
                          style={{ padding: 16, background: 'rgba(0,0,0,0.4)' }}
                        >
                          {caja.items.map((item, idx) => (
                            <div
                              key={item.itemId}
                            ...
                            >
                              ...
                              >
                                <button
                                  ...
                                  onClick={() =>
                                    actualizarItemCaja(cajaId, idx, 'ok', true)
                                  }
                                >
                                  ✓ OK
                                </button>
                                <button
                                  ...
                                  onClick={() =>
                                    actualizarItemCaja(cajaId, idx, 'ok', false)
                                  }
                                >
                                  ✗ NO OK
                                </button>
                ...
                {Object.keys(estadoCajas).length === 0 && (
                  <p style={{ color: '#94a3b8' }}>No hay cajas asignadas</p>
                )}
              </div>
            )}
          ...
          <button
            ...
            style={{
              ...
              background: guardando
                ? '#475569'
                : 'linear-gradient(95deg, #10b981, #059669)',
              color: 'white',
              border: 'none',
              borderRadius: 60,
              fontWeight: 'bold',
